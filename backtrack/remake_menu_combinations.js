...
function solution(orders, course) {
  const answer = [];
  const menuCount = new Map(); // 메뉴 조합별 주문 횟수
  ...
  for (const order of orders) {
    ...
    for (const size of course) {
      if (order.length >= size) {
        const combinations = getCombinations(sortedOrder.split(""), size);

        for (const combo of combinations) {
          const menuCombo = combo.join("");
          menuCount.set(menuCombo, (menuCount.get(menuCombo) || 0) + 1);
        }
      }
    }
  }

  // 각 코스 길이별로 최대 주문 횟수 찾기
  for (const size of course) {
    const maxCount = Math.max(
      ...Array.from(menuCount.entries())
        .filter(([menu]) => menu.length === size)
        .map(([, count]) => count)
    );

    // 최소 2명 이상 주문된 조합만 고려
    if (maxCount >= 2) {
      // 해당 길이에서 최대 주문 횟수를 가진 메뉴들 추가
      for (const [menu, count] of menuCount.entries()) {
        if (menu.length === size && count === maxCount) {
          answer.push(menu);
        }
      }
    }
